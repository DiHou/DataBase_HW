...
FROM PRODUCT WHERE EIN IS NULL;

CREATE TABLE ANSWER01
SELECT DISTINCT * FROM TEMP1 ORDER BY PRODUCTNAME ASC, PRODUCTID ASC;
...
CREATE TABLE TEMP2
SELECT EIN, NAME FROM COMPANY;

CREATE TABLE TEMP3
SELECT EIN FROM CUSTOMER;

INSERT INTO TEMP3
SELECT EIN FROM SHIPPER;
INSERT INTO TEMP3
SELECT EIN FROM VENDOR;

CREATE TABLE TEMP4
SELECT * FROM TEMP2
WHERE NOT EXISTS (SELECT EIN FROM TEMP3 WHERE TEMP2.EIN=TEMP3.EIN);

CREATE TABLE ANSWER02
SELECT DISTINCT * FROM TEMP4 ORDER BY EIN ASC, NAME ASC;
...
FROM PRODUCT WHERE WEIGHTINLB>10;

CREATE TABLE ANSWER03 
SELECT DISTINCT * FROM TEMP5 ORDER BY AVERAGE_PRICE ASC;
...
CREATE TABLE TEMP6
SELECT NAME AS CUSTOMER_NAME, CREDIT AS CREDIT_RATING, PRODUCTNAME AS PRODUCT_NAME
FROM COMPANY, CUSTOMER, ORDERS
WHERE COMPANY.EIN=CUSTOMER.EIN AND ORDERS.CUSTOMEREIN=CUSTOMER.EIN;

CREATE TABLE ANSWER04
SELECT DISTINCT * FROM TEMP6 ORDER BY CUSTOMER_NAME ASC, CREDIT_RATING ASC, PRODUCT_NAME ASC;
...
CREATE TABLE TEMP7
SELECT SHIPPEREIN, COUNT(SHIPPEREIN) AS WAREHOUSE_NUMBER
FROM WAREHOUSE GROUP BY SHIPPEREIN;

CREATE TABLE TEMP8
SELECT SHIPPEREIN FROM TEMP7
WHERE WAREHOUSE_NUMBER>2;

CREATE TABLE TEMP9
SELECT NAME AS SHIPPER_NAME, TEMP8.SHIPPEREIN AS SHIPPER_EIN, PRICEPERLB AS PRICE_LB
FROM COMPANY, TEMP8, SHIPPER
WHERE COMPANY.EIN=TEMP8.SHIPPEREIN AND SHIPPER.EIN=TEMP8.SHIPPEREIN;

CREATE TABLE ANSWER05
SELECT DISTINCT * FROM TEMP9 ORDER BY SHIPPER_NAME DESC, SHIPPER_EIN DESC, PRICE_LB DESC;
...
CREATE TABLE TEMP10
SELECT PRODUCTUSAGE.PRODUCTNAME, PRODUCTUSAGE.PRODUCTID, PRODUCTUSAGEVALUE, PRICE
FROM PRODUCTUSAGE, PRODUCT
WHERE PRODUCTUSAGE.PRODUCTID=PRODUCT.PRODUCTID;

CREATE TABLE TEMP11
SELECT * FROM TEMP10;

CREATE TABLE TEMP12
SELECT TEMP10.PRODUCTNAME AS PRODUCT_1, TEMP11.PRODUCTNAME AS PRODUCT_2
FROM TEMP10, TEMP11
WHERE TEMP10.PRODUCTUSAGEVALUE>TEMP11.PRODUCTUSAGEVALUE AND TEMP10.PRICE<TEMP11.PRICE AND TEMP10.PRODUCTID<>TEMP11.PRODUCTID;

CREATE TABLE ANSWER06
SELECT DISTINCT PRODUCT_1, PRODUCT_2 FROM TEMP12 ORDER BY PRODUCT_1 ASC, PRODUCT_2 ASC;
...
CREATE TABLE TEMP13
SELECT NAME, CUSTOMEREIN, PRODUCTID FROM COMPANY, ORDERS
WHERE COMPANY.EIN=ORDERS.CUSTOMEREIN AND COMPANY.NAME<>'IBM';
...
CREATE TABLE TEMP15
SELECT * FROM TEMP13
WHERE EXISTS (SELECT CUSTOMEREIN FROM TEMP14 WHERE TEMP13.PRODUCTID=TEMP14.PRODUCTID);

CREATE TABLE TEMP16
SELECT DISTINCT NAME, CUSTOMEREIN, COUNT(PRODUCTID) AS NUMBER FROM TEMP14 GROUP BY NAME;

CREATE TABLE TEMP17
SELECT DISTINCT NAME, CUSTOMEREIN, COUNT(PRODUCTID) AS NUMBER FROM TEMP15 GROUP BY NAME;

CREATE TABLE ANSWER07
SELECT DISTINCT CUSTOMEREIN AS CUSTOMER_EIN, NAME AS CUSTOMER_NAME FROM TEMP17
WHERE TEMP17.NUMBER=(SELECT NUMBER FROM TEMP16) ORDER BY CUSTOMER_EIN ASC, CUSTOMER_NAME ASC;
...
CREATE TABLE TEMP18
SELECT CONTAINS.PRODUCTNAME, PRICE, ITEMSIZE FROM CONTAINS, PRODUCT, ITEM
WHERE PRODUCT.PRODUCTID=CONTAINS.PRODUCTID AND ITEM.ITEMID=CONTAINS.ITEMID;

CREATE TABLE ANSWER08
SELECT DISTINCT PRODUCTNAME AS PRODUCT_NAME, PRICE, ITEMSIZE FROM TEMP18 AS TEMP19
WHERE TEMP19.ITEMSIZE>=ALL (SELECT ITEMSIZE FROM TEMP18)
ORDER BY PRODUCTNAME ASC, PRICE ASC, ITEMSIZE ASC;
...
CREATE TABLE TEMP19
SELECT NAME, CUSTOMEREIN, SHIPPEREIN, ORDERS.PRODUCTID, QUANTITY, WEIGHTINLB, PRICEPERLB FROM COMPANY, ORDERS, PRODUCT, SHIPPER
WHERE COMPANY.EIN=ORDERS.CUSTOMEREIN AND ORDERS.SHIPPEREIN=SHIPPER.EIN AND ORDERS.PRODUCTID=PRODUCT.PRODUCTID;

CREATE TABLE TEMP20
SELECT NAME AS CUSTOMER_NAME, (QUANTITY*WEIGHTINLB*PRICEPERLB) AS SHIPMENT_PRICE FROM TEMP19;

CREATE TABLE ANSWER09
SELECT DISTINCT CUSTOMER_NAME, SUM(SHIPMENT_PRICE) AS SHIPMENT_PRICE FROM TEMP20
